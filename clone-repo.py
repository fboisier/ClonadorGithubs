...
while True:
    indice = input("Ingresar trabajo a revisar: ")
    if int(indice) < len(trabajo):
        trabajo = trabajo[int(indice)]
        break
    else:
        print("ingresar numero correcto.")

log_p("Abriendo archivo repositorios")
...
for linea in reader:
    ...
    try:
        ...
        git.Repo.clone_from(f'https://github.com/{repo}/{trabajo}', carpetaLocal)
        log_p("Exito en Clonación de " + alumno)
        exito+=1
    except:
        log_p("---------------> Alumno no sube trabajo: " + alumno)
        log_p(f'---------------> https://github.com/{repo}/{trabajo}')
        log_f(alumno)
...
log_p("-"*10 + " ALUMNOS CON ENTREGA: " + str(exito) + " " + "-"*10)
log_p("*"*35)
